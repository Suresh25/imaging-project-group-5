...
while waar == true
    ...
    n = normalise(data(:,:,:,1));
    s = segmentation(n);
    %s = loglog(diphist(s(:,:,1)),diphist(s(:,:,2)), 'bo');
    %hist(handles.axes4, y, 70);
    l = labeling(s);
    p = property(l);
    c = classification(p);
    t = count(c);
    ...
    tm1 = data(:,:,1,1);
    tm2 = data(:,:,2,1);
    %scatter(handles.axes1, reshape(tm1.',1,[]), reshape(tm2.',1,[]), 'bo');
    scatter(handles.axes1, diphist(tm1), diphist(tm2), 'bo');
...
temp = dm;
	...
	imshow(temp.*n, 'Parent', handles.axes3); %set(h, 'CData', n);
    
    %h = get(handles.axes4, 'Children');
	%set(h, 'CData', s);
    semilogy(handles.axes4, linspace(0, 255, 256), diphist(s));
    xlim(handles.axes4, [0 255]);
